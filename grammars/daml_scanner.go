...
func (DamlExternalScanner) Create() any {
	payload := HaskellExternalScanner{}.Create()
	// Mark the state as DAML so the shared scanner declines the one
	// classification the two dialects genuinely disagree about: a lone
	// spaced colon is DAML's type annotation, not an infix data constructor.
	if st, ok := payload.(*hsState); ok {
		st.damlDialect = true
	}
	return payload
}
