...
module.exports = {
  _cairo_1_statement: $ => choice(
    // `component!(path: X, storage: y, event: Z);` — a macro used as an
    // item-level statement inside a contract module.
    $.macro_statement,
    $.use_declaration,
  ...
  // Cairo 2.x visibility. The upstream grammar scaffolded this at every
  // item site but never defined the rule, leaving all ten commented out.
  visibility_modifier: $ =>
    prec.right(seq('pub', optional(seq('(', 'crate', ')')))),
  ...
  // `+Drop<T>` / `-Drop<T>`: Cairo 2.x sugar for an anonymous impl bound in
  // a generic parameter list.
  impl_bound_parameter: $ =>
    seq(choice('+', '-'), $._cairo_1_type),

  // `array![...]`, `selector!("...")`, `component!(path: X, ...)`. Cairo
  // macros take any of the three bracket styles and arbitrary token content,
  // so the body is matched as balanced token trees rather than parsed as an
  // expression — the upstream grammar scaffolded this but never defined it.
  macro_statement: $ => prec.right(21, seq($.macro_invocation, optional(';'))),
  ...
  token_tree: $ => choice(
    seq('(', repeat($._token_tree_item), ')'),
    seq('[', repeat($._token_tree_item), ']'),
    seq('{', repeat($._token_tree_item), '}'),
  ),

  _token_tree_item: $ => choice(
    $.token_tree,
    $._non_delimiter_token,
  ),

  _non_delimiter_token: _ =>
    token(choice(
      /[^()\[\]{}"]+/,
  ...
  type_parameters: $ => prec(1, seq(
    ...
    sep1(
      choice(
        // $.lifetime,
        // $.metavariable,
        // Cairo 2.x anonymous impl bound: `<T, +Drop<T>>`.
        $.impl_bound_parameter,
        $._cairo_1_type_identifier,
